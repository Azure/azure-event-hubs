...
**SRS_EVENTHUBRECEIVER_LL_29_105: \[**If connection_string_parser_parse fails then `EventHubReceiver_Create_LL` shall fail and return NULL.**\]**
...
```
**SRS_EVENTHUBRECEIVER_LL_29_301: \[**`EventHubReceiver_ReceiveFromStartTimestampAsync_LL` shall fail and return EVENTHUBRECEIVER_INVALID_ARG if parameter eventHubReceiverHandle, onEventReceiveErrorCallback, onEventReceiveErrorCallback are NULL.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_302: \[**`EventHubReceiver_ReceiveFromStartTimestampAsync_LL` shall record the callbacks and contexts in the EVENTHUBRECEIVER_LL_STRUCT.**\]** 
**SRS_EVENTHUBRECEIVER_LL_29_303: \[**If cloning and/or adding the information fails for any reason, `EventHubReceiver_ReceiveFromStartTimestampAsync_LL` shall fail and return EVENTHUBRECEIVER_ERROR.**\]**
...
**SRS_EVENTHUBRECEIVER_LL_29_307: \[**Transition state of eventHubReceiverHandle to RECEIVE**\]**
**SRS_EVENTHUBRECEIVER_LL_29_308: \[**Otherwise `EventHubReceiver_ReceiveFromStartTimestampAsync_LL` shall succeed and return EVENTHUBRECEIVER_OK.**\]**

### EventHubReceiver_ReceiveFromStartTimestampWithTimeoutAsync_LL
```c
MOCKABLE_FUNCTION(, EVENTHUBRECEIVER_RESULT, EventHubReceiver_ReceiveFromStartTimestampWithTimeoutAsync_LL,
	EVENTHUBRECEIVER_LL_HANDLE, eventHubReceiverLLHandle,
	EVENTHUBRECEIVER_ASYNC_CALLBACK, onEventReceiveCallback,
...
```
**SRS_EVENTHUBRECEIVER_LL_29_401: \[**`EventHubReceiver_ReceiveFromStartTimestampWithTimeoutAsync_LL` shall fail and return EVENTHUBRECEIVER_INVALID_ARG if parameter eventHubReceiverHandle, onEventReceiveErrorCallback, onEventReceiveErrorCallback are NULL.**\]** 
**SRS_EVENTHUBRECEIVER_LL_29_402: \[**`EventHubReceiver_ReceiveFromStartTimestampWithTimeoutAsync_LL` shall record the callbacks and contexts in the EVENTHUBRECEIVER_LL_STRUCT.**\]** 
**SRS_EVENTHUBRECEIVER_LL_29_403: \[**If cloning and/or adding the information fails for any reason, `EventHubReceiver_ReceiveFromStartTimestampWithTimeoutAsync_LL` shall fail and return EVENTHUBRECEIVER_ERROR.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_304: \[**Create a filter string using format "apache.org:selector-filter:string" and "amqp.annotation.x-opt-enqueuedtimeutc > startTimestampInSec" using STRING_sprintf**\]**
**SRS_EVENTHUBRECEIVER_LL_29_305: \[**If filter string create fails then a log message will be logged and an error code of EVENTHUBRECEIVER_ERROR shall be returned.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_306: \[**Initialize timeout value (zero if no timeout) and a current timestamp of now.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_308: \[**Otherwise `EventHubReceiver_ReceiveFromStartTimestampWithTimeoutAsync_LL` shall succeed and return EVENTHUBRECEIVER_OK.**\]**
...
**SRS_EVENTHUBRECEIVER_LL_29_570: \[**`EventHubReceiver_LL_DoWork` shall call messagereceiver_create and pass link created in SRS_EVENTHUBRECEIVER_LL_29_550 `EventHubReceiver_LL_OnStateChanged` as the ON_MESSAGE_RECEIVER_STATE_CHANGED parameter and the EVENTHUBRECEIVER_LL_HANDLE as the callback context**\]**
**SRS_EVENTHUBRECEIVER_LL_29_571: \[**Once the message receiver is created, `EventHubReceiver_LL_DoWork` shall call messagereceiver_open and pass in `EventHubReceiver_LL_OnMessageReceived` as the ON_MESSAGE_RECEIVED parameter and the EVENTHUBRECEIVER_LL_HANDLE as the callback context**\]**
**SRS_EVENTHUBRECEIVER_LL_29_572: \[**If event data was received, the user receive callback should be issued along with the data and result code EVENTHUBRECEIVER_OK**\]**

##### EventHubReceiver_LL_OnStateChanged
```c
static void EventHubReceiver_LL_OnStateChanged(const void* context, MESSAGE_RECEIVER_STATE newState, MESSAGE_RECEIVER_STATE previousState)
```
**SRS_EVENTHUBRECEIVER_LL_29_600: \[**When `EventHubReceiver_LL_OnStateChanged` is invoked, obtain the EventHubReceiverLL handle from the context and update the message receiver state with the new state received in the callback.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_601: \[**If the new state is MESSAGE_RECEIVER_STATE_ERROR, and previous state is not MESSAGE_RECEIVER_STATE_ERROR, `EventHubReceiver_LL_OnStateChanged` shall invoke the user supplied error callback along with error callback context`**\]**


##### EventHubReceiver_LL_OnMessageReceived
```c
static AMQP_VALUE EventHubReceiver_LL_OnMessageReceived(const void* context, MESSAGE_HANDLE message)
```
**SRS_EVENTHUBRECEIVER_LL_29_700: \[**When `EventHubReceiver_LL_OnMessageReceived` is invoked, message_get_body_amqp_data shall be called to obtain the data into a BINARY_DATA buffer.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_702: \[**`EventHubReceiver_LL_OnMessageReceived` shall create a EVENT_DATA handle using EventData_CreateWithNewMemory and pass in the buffer data pointer and size as arguments.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_703: \[**`EventHubReceiver_LL_OnMessageReceived` shall obtain the application properties using message_get_application_properties() and populate the EVENT_DATA handle map with these key value pairs.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_704: \[**`EventHubReceiver_LL_OnMessageReceived` shall obtain event data specific properties using message_get_properties() and populate the EVENT_DATA handle with these properties.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_701: \[**If any errors are seen `EventHubReceiver_LL_OnMessageReceived` shall reject the incoming message by calling messaging_delivery_rejected() and return.**\]**
**SRS_EVENTHUBRECEIVER_LL_29_701: \[**`EventHubReceiver_LL_OnMessageReceived` shall invoke the user registered onMessageReceive callback with status code EVENTHUBRECEIVER_OK, the EVENT_DATA handle and the context passed in by the user.**\]**
...
MOCKABLE_FUNCTION(, EVENTHUBRECEIVER_RESULT, EventHubReceiver_Set_ConnectionTracing_LL,
                    EVENTHUBRECEIVER_LL_HANDLE, eventHubReceiverLLHandle, bool, traceEnabled);
```
**SRS_EVENTHUBRECEIVER_LL_29_800: \[**`EventHubReceiver_Set_ConnectionTracing_LL` shall fail and return EVENTHUBRECEIVER_INVALID_ARG if parameter eventHubReceiverLLHandle.**\]** 
