...
public final class PartitionReceiver extends ClientEntity
{
	 ...
	 * @param invokeOnTimeout flag to indicate whether the {@link PartitionReceiveHandler#onReceive(Iterable)} should be invoked when the receive call times out
	 */
	public void setReceiveHandler(final PartitionReceiveHandler receiveHandler, final boolean invokeOnTimeout)
	{
		synchronized (this.receiveHandlerSync)
		{
			this.invokeHandlerOnTimeout = invokeOnTimeout;
			...
			else
			{
				if (this.isOnReceivePumpRunning)
					throw new IllegalArgumentException(
							"Unexpected value for parameter 'receiveHandler'. Close the existing running pump and then set the new handler");
